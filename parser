...
# Fiscal-year-aware month ordering (Apr = 1 … Mar = 12)
MONTH_ORDER = {
...
# Keyword → standard column name mapping
KEYWORD_MAP = {
...
def normalize_month(raw: str) -> str | None:
    ...
    Handles: 'April', 'apr', 'Apr-24', 'April 2024', '04/2024', '2024-04-15', etc.
    ...
    # Try numeric month from date strings: '2024-04-15', '15/04/2024', '04/2024'
    patterns = [
        r'\d{4}[\-/](\d{1,2})[\-/]\d{1,2}',  # 2024-04-15
        r'\d{1,2}[\-/](\d{1,2})[\-/]\d{4}',  # 15/04/2024
        r'(\d{1,2})[\-/]\d{4}',               # 04/2024
    ]
    for pat in patterns:
        m = re.search(pat, raw)
        if m:
            ...
            if 1 <= month_num <= 12:
                abbr = list(MONTH_ALIASES.values())[month_num - 1]
                # Use ordered unique list
                rev = {v: v for v in MONTH_ORDER}
                num_to_abbr = {
...
def sort_months(months: list) -> list:
    """Sort month abbreviations in fiscal year order (Apr first)."""
    return sorted(months, key=lambda m: MONTH_ORDER.get(m, 99))


def extract_month_from_date(date_val) -> str | None:
    """Extract normalized month from a pandas date or string."""
    try:
        if pd.isna(date_val):
            return None
...
def map_columns(df: pd.DataFrame) -> dict:
    """
    Scan DataFrame column headers and map them to standard names
    using keyword matching. Returns {standard_name: actual_col_name}.
    """
    mapping = {}
...
def safe_numeric(series: pd.Series) -> pd.Series:
    """Coerce a column to numeric, replacing errors with 0."""
...
def parse_books_excel(file_bytes: bytes, label: str = "Books") -> pd.DataFrame:
    """
    Parse a Books Excel file (Sales / Purchase / Journal Register).
    Detects month either from a 'Month' column or from Invoice Date.
    Returns a standardized DataFrame with columns:
      month, sales_value, export_value, sez_value, igst, cgst, sgst,
    ...
    try:
        ...
        for sheet in xl.sheet_names:
            ...
            # Find header row: first row with >= 3 non-null text cells
            header_row = 0
...
def parse_credit_ledger_excel(file_bytes: bytes) -> pd.DataFrame:
    """
    Parse Electronic Credit Ledger Excel.
    Column F (index 5) contains 'Credit' or 'Debit'.
    Returns DataFrame with: month, entry_type, igst, cgst, sgst
    """
    try:
        xl = pd.ExcelFile(BytesIO(file_bytes))
        frames = []
        ...
        for sheet in xl.sheet_names:
            ...
            # Column F (index 5) = credit/debit type
            col_f = df.iloc[:, 5] if df.shape[1] > 5 else pd.Series([''] * len(df))
            
            # Also check mapped 'note_type' column
            ...
            for field in ['igst', 'cgst', 'sgst']:
                if field in col_map:
                    ...
                    # Try positional: IGST col 3, CGST col 4, SGST col 5 (0-based)
...
def parse_gstr2b_excel(file_bytes: bytes) -> dict:
    ...
    Returns dict with keys: 'b2b', 'b2b_cdnr', 'impz'
    Each value is a DataFrame with: month, igst, cgst, sgst (+ invoice_no, gstin for Module 4)
    """
    ...
    try:
        ...
        for target_key, aliases in [
            ...
            if 'invoice_date' in col_map:
                ...
                # Try col index 3 (common for GSTR-2B B2B)
                if df.shape[1] > 3:
            ...
            if target_key == 'b2b_cdnr':
                if 'note_type' in col_map:
                    ...
                    std_df['note_type'] = 'Debit'  # default assumption
...
def parse_books_pdf(file_bytes: bytes, label: str = "Books") -> pd.DataFrame:
    """Parse a Books PDF (sales data, purchase data) using pdfplumber."""
...
def parse_gstr1_pdf(file_bytes: bytes) -> pd.DataFrame:
    """
    Parse GSTR-1 PDF from GST Portal.
    Extracts month-wise: sales_value, export_value, sez_value, igst, cgst, sgst.
    Handles the portal's standard summary and detail table formats.
    """
    tables = _extract_pdf_tables(file_bytes)
    frames = []

    for df in tables:
        df_lower_cols = [str(c).lower().strip() for c in df.columns]
        has_tax = any(kw in ' '.join(df_lower_cols) for kw in ['igst', 'cgst', 'sgst', 'integrated', 'central', 'state'])
        has_val = any(kw in ' '.join(df_lower_cols) for kw in ['value', 'amount', 'taxable'])
...
def aggregate_monthly(df: pd.DataFrame, value_cols: list) -> pd.DataFrame:
    """
    Group a parsed DataFrame by month and sum value columns.
    ...
    # Fill missing value cols with 0
